"""Visualization utilities for floor plan rendering.

Provides functions for drawing floor plans and their components using Matplotlib,
with color mappings for different room types.
"""

import matplotlib.pyplot as plt
from matplotlib.patches import Polygon

import numpy as np

from src.floor_plan import FloorPlan, RoomType, Room

pastel_colors = [
    "#A6C8E0",
    "#F4B6A6",
    "#B8D8BA",
    "#F2C1C1",
    "#C6B7E2",
    "#D3B8AE",
    "#F0CDE3",
    "#CFCFCF",
    "#D8E2A8",
    "#BFE4E8",
    "#E1ECF7",
    "#FFE0B5",
    "#CFEBC7"
]

_room2color = {
    RoomType.LivingRoom: pastel_colors[0],
    RoomType.MasterRoom: pastel_colors[1],
    RoomType.Kitchen:    pastel_colors[2],
    RoomType.Bathroom:   pastel_colors[3],
    RoomType.DiningRoom: pastel_colors[4],
    RoomType.ChildRoom:  pastel_colors[5],
    RoomType.StudyRoom:  pastel_colors[6],
    RoomType.SecondRoom: pastel_colors[7],
    RoomType.GuestRoom:  pastel_colors[8],
    RoomType.Balcony:    pastel_colors[9],
    RoomType.Entrance:   pastel_colors[10],
    RoomType.Storage:    pastel_colors[11],
    RoomType.WallIn:     pastel_colors[12]
}


room2str = {
    RoomType.LivingRoom: "Living Room",
    RoomType.MasterRoom: "Master Room",
    RoomType.Kitchen:    RoomType.Kitchen.name,
    RoomType.Bathroom:   RoomType.Bathroom.name,
    RoomType.DiningRoom: "Dining Room",
    RoomType.ChildRoom:  "Child Room",
    RoomType.StudyRoom:  "Study Room",
    RoomType.SecondRoom: "Second Room",
    RoomType.GuestRoom:  "Guest Room",
    RoomType.Balcony:    RoomType.Balcony.name,
    RoomType.Entrance:   RoomType.Entrance.name,
    RoomType.Storage:    RoomType.Storage.name,
    RoomType.WallIn:     "Wall In"
}


def _draw_doors(ax, floor_plan: FloorPlan):
    """Draw front door on axes.
    
    :param ax: Matplotlib axes object
    :param floor_plan: Floor plan object
    :type floor_plan: FloorPlan
    """

    x = floor_plan.front_door.xs
    y = floor_plan.front_door.ys

    ax.plot(x, y, linewidth=2, color="yellow")


def _draw_room_boundary(ax, room: Room):
    """Draw room polygon boundaries on axes.
    
    :param ax: Matplotlib axes object
    :param room: Room to be drawn
    :type room: Room
    """

    x = room.boundary[:, 0]
    y = room.boundary[:, 1]

    x = np.append(x, room.boundary[0, 0])
    y = np.append(y, room.boundary[0, 1])

    ax.plot(x, y, linewidth=2, color="grey")


def _draw_boundary(ax, floor_plan: FloorPlan):
    """Draw the outer boundary of a floor plan on axes.
    
    :param ax: Matplotlib axes object
    :param floor_plan: Floor plan whose boundary should be drawn
    """

    x = floor_plan.boundary[:, 0]
    y = floor_plan.boundary[:, 1]

    x = np.append(x, floor_plan.boundary[0, 0])
    y = np.append(y, floor_plan.boundary[0, 1])

    ax.plot(x, y, linewidth=2, color="black")


def _draw_room_corners(ax, floor_plan: FloorPlan):
    """Draw markers at every corner point of rooms on axes.
    
    :param ax: Matplotlib axes object
    :param floor_plan: Floor plan object
    :type floor_plan: FloorPlan
    """

    for room in floor_plan.rooms:
        x = room.boundary[:, 0]
        y = room.boundary[:, 1]

        ax.scatter(x, y, color="black", s=10)



def draw_floor_plan(floor_plan: FloorPlan, draw_room_corners: bool = False):
    """Draw a floor plan on the screen with all components.
    
    Draws boundary, rooms (colored by type), front door, and optionally room corners.
    
    :param floor_plan: Floor plan object to draw
    :param draw_room_corners: Whether to draw corner markers (default: False)
    """

    fig, ax = plt.subplots()

    for room in floor_plan.rooms:
        p = Polygon(room.boundary, facecolor=_room2color[room.type], label=room2str[room.type])

        ax.add_patch(p)

    for room in floor_plan.rooms:
        _draw_room_boundary(ax, room)

    _draw_boundary(ax, floor_plan)
    _draw_doors(ax, floor_plan)

    if draw_room_corners:
        _draw_room_corners(ax, floor_plan)

    plt.grid()
    plt.ylim(0, 250)
    plt.xlim(0, 250)
    plt.gca().invert_yaxis()

    # Remove duplicate legend entries
    handles, labels = ax.get_legend_handles_labels()
    unique = [(h, l) for i, (h, l) in enumerate(zip(handles, labels)) if l not in labels[:i]]
    ax.legend(*zip(*unique))

    plt.title(f'Floor plan {floor_plan.name}')

    plt.show()


def draw_floor_plan_to_image(floor_plan: FloorPlan, filename: str, with_legend: bool=True):
    """
    Saves floor plan as image file
    
    :param floor_plan: Floor plan object to be saved
    :type floor_plan: FloorPlan
    :param filename: The output path (e.g., 'plan.png'). If file does not exist new one is created.
    :type filename: str
    :param with_legend: Whether to include a color-coded key for room types. 
                        Defaults to True.
    :type with_legend: bool
    """

    fig, ax = plt.subplots()

    ax.axis("off")

    for room in floor_plan.rooms:
        p = Polygon(room.boundary, facecolor=_room2color[room.type], label=room2str[room.type])

        ax.add_patch(p)

    for room in floor_plan.rooms:
        _draw_room_boundary(ax, room)

    _draw_boundary(ax, floor_plan)
    _draw_doors(ax, floor_plan)

    plt.ylim(0, 250)
    plt.xlim(0, 250)
    plt.gca().invert_yaxis()

    if with_legend:
        # Remove duplicate legend entries
        handles, labels = ax.get_legend_handles_labels()
        unique = [(h, l) for i, (h, l) in enumerate(zip(handles, labels)) if l not in labels[:i]]
        ax.legend(*zip(*unique))

    plt.savefig(filename, bbox_inches='tight')

    plt.close()


def draw_legend_to_file(filename: str, rooms: list[RoomType] | None=None):
    """
    Creates file with all room colors.
    
    :param filename: Output file (e.g., 'legend.png')
    :type filename: str
    """

    fig, ax = plt.subplots()

    dummy_boundary = [
        (0, 0),
        (0, 1),
        (1, 1), 
        (1, 0)
    ]

    handles = []

    if rooms is None:
        for room in RoomType:
            p = Polygon(dummy_boundary, facecolor=_room2color[room], label=room2str[room])

            handles.append(p)
    else:
        for room in rooms:
            p = Polygon(dummy_boundary, facecolor=_room2color[room], label=room2str[room])

            handles.append(p)

    ax.legend(handles=handles, frameon=False, loc='center')
    ax.axis('off')

    fig.savefig(filename, bbox_inches='tight')
    


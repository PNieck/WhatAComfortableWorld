"""Training configuration management for floor plan models.

Provides TrainingConfig class for loading, storing, and accessing training
configuration parameters from YAML files. Includes properties for model,
...
class CyclicLRSchedulerConfig:
    ...
    :ivar base_lr: Minimum learning rate
    :ivar max_lr: Maximum learning rate
    :ivar cycles: Number of cycles
    """
    base_lr: float
    max_lr: float
    cycles: int
...
class TrainingConfig:
    ...
    Loads YAML configuration files and provides properties to access model,
    training, and logging settings. Validates and preprocesses configuration
    ...
    @property
    def model_has_vertex_indices(self) -> bool:
        return self.model_config["with_vertex_indices"]
    ...
    def __init__(self, path: str|None = None):
        """Initialize TrainingConfig from a YAML file or with empty configuration.
        
        :param path: Path to YAML configuration file (default: None for empty config)
        """
        if path is None:
            self.general_config = {}
            self.model_config = {}
    ...
    def update_with_tokenizer(self, tokenizer):
        """Update model vocabulary size based on tokenizer.
        
        :param tokenizer: Tokenizer object with __len__ method
        """
    ...
    def state_dict(self) -> dict:
        ...
        :return: Dictionary representation of configuration state
        """
        result = {}
    ...
    def load_state_dict(self, d: dict):
        ...
        Preprocesses model and training configurations and sets up logging directory.
        
        :param d: Configuration dictionary with keys "general", "model", "training", "logging"
        """
        if "general" not in d:
    ...
    def _preprocess_model_config(self):
        """Preprocess model configuration boolean entries."""
    ...
    def _preprocess_train_config(self):
        """Preprocess training configuration and set up learning rate scheduler."""
    ...
    def _preprocess_bool_entry(config: dict, entry: str):
        """Ensure boolean config entry exists, defaulting to False if absent.
        
        :param config: Configuration dictionary
        :param entry: Key name for boolean entry
        """
    ...
    def _get_log_dir(self) -> str:
        """Generate logging directory path with timestamp.
